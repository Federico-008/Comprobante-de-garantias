...
export default function PublicWarrantyView() {
  ...
  const fetchData = () => {
    try {
      const g = storage.getComprobanteById(id as string);
  ...
  if (!garantia || !perfil) return (
    <div className="min-h-screen flex flex-col items-center justify-center bg-background gap-4">
      <div className="bg-red-50 dark:bg-red-500/10 border border-red-200 dark:border-red-500/20 rounded-3xl p-8 text-center max-w-sm">
        ...
        <p className="text-obsidian-500 text-sm mt-2">El documento no existe o ha sido eliminado del dispositivo local.</p>
        <Link href="/dashboard" className="inline-block mt-4 text-sapphire-600 font-bold hover:underline">Ir al Dashboard</Link>
      </div>
    </div>
  );
